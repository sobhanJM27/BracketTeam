...
const AdminCategories = () => {
  ...
  const addCategoryMutation = useMutation({
    mutationFn: (newCategory) =>
      addCategory(
        { token, ...auth },
        newCategory.fa.titleFa,
        newCategory.en.titleEn
      ),
    onSuccess: () => {
  ...
  const editCategoryMutation = useMutation({
    mutationFn: (updatedCategory) =>
      editCategory(
        { token, ...auth },
        editingCategoryId,
        updatedCategory.fa.titleFa,
        updatedCategory.en.titleEn
      ),
    onSuccess: () => {
      queryClient.invalidateQueries(['categories']);
            ...
            categories.map((category) => (
              <li key={category._id}>
                <input
                  ...
                  onChange={(e) =>
                    handleCategoryChange(category._id, 'fa', e.target.value)
                  }
                />
                <input
                  type="text"
                  ...
                  onChange={(e) =>
                    handleCategoryChange(category._id, 'en', e.target.value)
                  }
                />
                <div>
                  <button
